...
/* Total number of bytes of combined X/Y/P registers*/
#define NUM_BYTES_PTC_PINREG (uint8_t)(4u)  

/* Aux pins - number of bytes depends on acq module type */
#define NUM_PINDEFS_SELFCAP				(uint8_t)(NUM_BYTES_PTC_PINREG)
#define NUM_PINDEFS_SHIELD              (uint8_t)(NUM_BYTES_PTC_PINREG << 1u)
#define NUM_PINDEFS_MUTUAL              (uint8_t)(NUM_BYTES_PTC_PINREG << 1u)
#define NUM_PINDEFS_MUTUAL_4P           (uint8_t)(NUM_BYTES_PTC_PINREG * 5u)
#define NUM_PINDEFS_MUTUAL_8P_CXT		(uint8_t)((8u + 2u) + NUM_BYTES_PTC_PINREG)
#define NUM_PINDEFS_MUTUAL_16P_CXT		(uint8_t)((16u + 2u) + NUM_BYTES_PTC_PINREG)
#define NUM_PINDEFS_MUTUAL_32P_CXT		(uint8_t)((32u + 2u) + NUM_BYTES_PTC_PINREG)
...
typedef struct
{
  ...
  uint8_t node_oversampling;							/* Oversampling */
...
{
  uint16_t num_node_sets;    /* Number of sensor node group */
  uint8_t acq_sensor_type;      /* Self or mutual sensors */
  uint8_t freq_option_select;   /* Frequency altering delay */
  uint8_t *node_sensor_pin_ptr;	/* Sensor and auxiliary pins: Mutual-X / Shield / Cext */
...
------------------------------------------------------------------------------
Purpose: Set wake-up exponent
Input  : Wake up exponent value
Output : none
Notes  : none
============================================================================*/
void qtm_set_wkupexp(uint8_t wakeup_exp, uint8_t node_prescaler);
...
#endif /* TOUCH_API_PIC32CZCA_GLOBAL_ACQ_H */
