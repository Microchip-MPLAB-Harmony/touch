...
/*============================================================================
Filename : cvd_driver_PIC32MZ : QTouch Modular Library
Purpose : Acquisition module - cvd_driver_PIC32MZ
------------------------------------------------------------------------------
------------------------------------------------------------------------------
Revision 1.0 - Initial Release 
============================================================================*/

#include "cvd_driver_PIC32MZ.h"
#include "definitions.h"
#include "touch.h" 
#include <math.h>

#if ENABLE_DRIVEN_SHIELD == 1u
#define CONCATE(a,b,c) a##b##c

#define OCMP_CompareSecondaryValueSet(index) CONCATE(OCMP,index,_CompareSecondaryValueSet(47+(ADCCON2bits.SAMC<<1)))
#define OCMP_Enable(index) CONCATE(OCMP,index,_Enable())
#define OCMP_Disable(index) CONCATE(OCMP,index,_Disable())
#define OCR(index)          CONCATE(OC,index,R)

#define TMR_Start(index)       CONCATE(TMR,index,_Start())
#define TMR_Stop(index)        CONCATE(TMR,index,_Stop())
#define TMR_PeriodSet(index)   CONCATE(TMR,index,_PeriodSet((ADCCON2bits.SAMC<<2)+92))
#define TMR(index)             CONCATE(TMR,index, )
#define TMR_IF(index)          CONCATE(IFS0bits.T,index,IF)
#endif
/* 
 * This table needs to be updated to the specific MCU 
 * Only class 2 and class 3 ADC input can be used 
 */
...
/* the RAM location for storing CVD data from DMA */
static __ADCCMPCON1bits_t __attribute__((coherent)) CVDData[64];
...
/*============================================================================
static void qtm_pic32_cvdDMA_handler_eoc(DMAC_TRANSFER_EVENT event, uintptr_t contextHandle)
------------------------------------------------------------------------------
Purpose: end of conversion interrupt
Input  : DMA event and contextHandler
Output : none
Notes  : none
============================================================================*/
void qtm_pic32_cvdDMA_handler_eoc(DMAC_TRANSFER_EVENT event, uintptr_t contextHandle);

/*============================================================================
static void cvd_config_io_pins(uint64_t cvd_lines,uint8_t direction, const uint8_t* pin_table)
------------------------------------------------------------------------------
...
touch_ret_t qtm_cvd_init_acquisition_module(qtm_acquisition_control_t* qtm_acq_control_ptr)
{
    ...
    {
        ADCIMCON1bits.DIFF7 = 0u;
        ADCCON3bits.DIGEN7 = 1u;

        ADCCMP1 = 0; // Clear the register
        /* Configure ADCCMPCONx */
        /* Set high and low threshold to 0 so that it generates 
           detect event and store the CVD result in ADCCMPCON1 register */
        ...
        ADCCMPCON1bits.IELOLO = 1;
        ADCCMPCON1bits.DCMPGIEN = 1; // enable interrupt
        ADCCMPCON1bits.ENDCMP = 0; // clear event bit
        ADCCMPCON1bits.ENDCMP = 1; // enable comparator
        ADCCON2bits.EOSIEN = 1;
        ADCCON1bits.STRGSRC = 0;
        
        DMAC_ChannelCallbackRegister(TOUCH_DMA_CHANNEL,qtm_pic32_cvdDMA_handler_eoc,0);
...
static void cvd_config_io_pins(uint64_t cvd_lines, uint8_t direction, const uint8_t pin_table[])
{
    ...
    for (local_count_var = 0u; local_count_var < 64u; local_count_var++)
    {
        /* XY Lines */
        if (0u == (cvd_lines & (uint64_t) ((uint64_t) 1ul << local_count_var)))
        {
...
static uint32_t get_cvd_result(void)
{
    int32_t touch_measurement_capture;
    /* the offset is to make sure the result of CVD is always positive 
       so that all the math afterwards will be unsigned math */
    uint32_t result_offset;
    uint8_t i,oversampling;
    
    oversampling = (uint8_t)1<<(qtm_acquisition_control_working_set_ptr->qtm_acq_node_config[current_measure_channel].node_oversampling);
    ...
    touch_measurement_capture = result_offset;
    
    for(i=0;i<oversampling;i++)
    {
        touch_measurement_capture+=(int32_t)((int16_t)CVDData[i].CVDDATA);
    }
    return (uint32_t)touch_measurement_capture;
}
/*============================================================================
static uint8_t qtm_load_group_config(void)
------------------------------------------------------------------------------
...
static void qtm_measure_node(uint16_t channel_number)
{
    ...
    {
        if (0u == ((qtm_acquisition_control_working_set_ptr->qtm_acq_node_config[channel_number].node_ymask) & ((uint64_t)1u << temp_var_mask_here)))
        {
        }
        else
        {
            if(temp_var_mask_here<32)
            {
                ADCCSS1 = (uint32_t) ((uint32_t)1<<temp_var_mask_here);
            }
            else
            {
                #ifdef ADCCSS2
                ADCCSS2 = (uint32_t) ((uint32_t)1<<(temp_var_mask_here-32));
                #endif
            }
            break;
        }
    ...
    /* Compensation cap */
    ADCCON2bits.CVDCPL = qtm_acquisition_control_working_set_ptr->qtm_acq_node_data[channel_number].node_comp_caps;
    ...
    temp_var_mask_here +=(sample_delay << 2u);
    
    if (temp_var_mask_here < (uint8_t) (NODE_CSD_MAX - CSD_MIN_OFFSET))
    {
        ADCCON2bits.SAMC = (temp_var_mask_here + CSD_MIN_OFFSET);
    }
    else
    {
        ADCCON2bits.SAMC = NODE_CSD_MAX;
    }

    oversampling = (uint8_t)1<<(qtm_acquisition_control_working_set_ptr->qtm_acq_node_config[channel_number].node_oversampling);
    
     /* 
     * Must read the whole 4 bytes of ADCCMPCON1, so that it will clear the 
     * the comparator event bit, otherwise, the DMA will keep getting triggered.
     */
    DMAC_ChannelTransfer(TOUCH_DMA_CHANNEL, (const void *)&ADCCMPCON1, 4, (const void *)&CVDData, (oversampling<<2), 4);

    /* Start Measurement */
#if ENABLE_DRIVEN_SHIELD == 1u
    TMR_IF(TOUCH_OC_TIMER_INDEX) = 0;
    TMR_PeriodSet(TOUCH_OC_TIMER_INDEX);
    OCMP_CompareSecondaryValueSet(TOUCH_OC_INDEX);
    TMR_Start(TOUCH_OC_TIMER_INDEX);
    OCMP_Enable(TOUCH_OC_INDEX);
    while(!TMR_IF(TOUCH_OC_TIMER_INDEX));
    TMR_Stop(TOUCH_OC_TIMER_INDEX);
    TMR(TOUCH_OC_TIMER_INDEX) = OCR(TOUCH_OC_INDEX) - 17 - ADCCON2bits.SAMC;
#endif
    ADCCON3bits.DIGEN7 = 1u;
    ADCCON1bits.CVDEN = 1u;
#if ENABLE_DRIVEN_SHIELD == 1u
    TMR_Start(TOUCH_OC_TIMER_INDEX);
#endif
}
...
/*============================================================================
uint16_t qtm_pic32mz_acq_module_get_id(void);
------------------------------------------------------------------------------
...
Notes  : none
============================================================================*/
uint16_t qtm_pic32mz_basic_acq_module_get_id(void)
{
    return QTM_MODULE_ID_PIC32MZ_BAISC_ACQ;
}

/*============================================================================
uint8_t qtm_pic32mz_acq_module_get_version(void);
------------------------------------------------------------------------------
...
Notes  : none
============================================================================*/
uint8_t qtm_pic32mz_basic_acq_module_get_version(void)
{
