...
/*============================================================================
Filename : hcvd_driver_PIC32CX : QTouch Modular Library
Purpose : Acquisition module - hcvd_driver_PIC32CX/HCVD
------------------------------------------------------------------------------
------------------------------------------------------------------------------
...
static uint8_t qtm_load_group_config(qtm_acquisition_control_t* qtm_acq_control_pointer)
{
    ...
    CVD_REGS->CVD_CVDCON = CVD_CVDCON_SDWREN(1);

    /* clear configuration for scan descriptor zero */
    CVD_REGS->CVD_CVDSD0C3 = 0;
    
    if (qtm_acquisition_control_working_set_ptr->qtm_acq_node_group_config->acq_sensor_type == NODE_MUTUAL)
    {
        CVD_REGS->CVD_CVDSD0C3 = CVD_CVDSD0C3_SDSELF(0) | CVD_CVDSD0C3_SDMUT(1);
    }
    else if (qtm_acquisition_control_working_set_ptr->qtm_acq_node_group_config->acq_sensor_type == NODE_SELFCAP)
    {
        CVD_REGS->CVD_CVDSD0C3 = CVD_CVDSD0C3_SDSELF(1) | CVD_CVDSD0C3_SDMUT(0);
    }
    else if (qtm_acquisition_control_working_set_ptr->qtm_acq_node_group_config->acq_sensor_type == NODE_SELFCAP_SHIELD)
    {
        CVD_REGS->CVD_CVDSD0C3 = CVD_CVDSD0C3_SDSELF(1) | CVD_CVDSD0C3_SDMUT(1);
    }
    else
    {
    ...
    if (param_ok_status == TOUCH_SUCCESS)
    {
        ...
        else
        {
            ...
            sample_delay = 0;//(uint8_t) (rand()&0x07u);
        }
    }

    /* Use PCLCK2 for the CVD core */
    CVD_REGS->CVD_CVDCON |= CVD_CVDCON_CLKSEL(0);
//    CVD_REGS->CVD_CVDCON &= ~CVD_CVDCON_SDWREN_Msk;
...
static void qtm_measure_node(uint16_t channel_number)
{
    ...
    uint8_t xy_counter;
    uint8_t num_x, num_y;
    uint32_t sd3con = CVD_REGS->CVD_CVDSD0C3;
...
//    CVD_REGS->CVD_CVDADC = 0b11u;
    ...
    CVD_REGS->CVD_CVDSD0C3 = sd3con;
    
    CVD_REGS->CVD_CVDSD0C3 |= CVD_CVDSD0C3_SDEN(1);
    
//    CVD_REGS->CVD_CVDCON &= CVD_CVDCON_SDWREN_Msk;

    CVD_REGS->CVD_CVDCON |= CVD_CVDCON_CVDIEN(1);
    CVD_REGS->CVD_CVDCON |= CVD_CVDCON_ON(1);
    
    /* Start Measurement */
    CVD_REGS->CVD_CVDCON |= CVD_CVDCON_SWTRIG(1);
}
...
touch_ret_t qtm_cvd_start_measurement_seq(qtm_acquisition_control_t* qtm_acq_control_pointer, void (*measure_complete_callback) (void))
{
    ...
    else if ((qtm_acq_control_pointer->qtm_acq_node_group_config->cvd_interrupt_priority) > 7u)
    {
        param_ok_status = TOUCH_INVALID_INPUT_PARAM;
    }
    else
    {
        NVIC_ClearPendingIRQ(CVD_IRQn);
        NVIC_SetPriority(CVD_IRQn, (uint32_t)qtm_acquisition_control_working_set_ptr->qtm_acq_node_group_config->cvd_interrupt_priority);
        NVIC_EnableIRQ(CVD_IRQn);
        /* Attach pointers */
