...
def getMaxGroups():
    """Get maximum scroller groups
    Arguments:
        :none
    Returns:
        :number of scroller groups  as (int)
    """
    global maxGroups
    return int(maxGroups)

def setMaxGroups(newMax):
    """Set maximum scroller groups
    Arguments:
...
def scrollerInstanceInitialise(qtouchComponent,groupNumber,parentLabel,selfChannels,mutualChannels):
    ...
    setScrollerEnableCountValues(touchSCRNum,mutualChannels)
    ...
    for channelID in range(0, mutualChannels):
        ...
        setScrollerTypeValues(touchScrollerType)
        setNumSegmentValues(touchScrollerNum,mutualChannels)
        setNumChannelsValues(touchScrollerNumChannels,mutualChannels)
        setResolutionValues(touchScrollerResolution)
        setDeadBandValues(touchScrollerDeadband)
        setPositionHysteresisValues(touchScrollerPosHysteresis)
        setContactThresholdValues(touchScrollerContactThreshold)


def initScrollerGroup(qtouchComponent, touchMenu, minVal, maxVal, selfChannels, mutualChannels):
    ...
    for i in range (minVal,maxVal+1):
        if i == 1:
            ...
            scrollerInstanceInitialise(qtouchComponent,i,scrollerMenu,selfChannels,mutualChannels)
        else:
            ...
            scrollerInstanceInitialise(qtouchComponent,i,vars()[dynamicName],selfChannels,mutualChannels)
    
    enableScrollerMenu.setDependencies(scrollerUpdateEnabled,["ENABLE_SCROLLER"])
    enableScrollerMenu.setValue(False)

def updateScrollerGroups(symbol,event):
...
def scrollerUpdateEnabled(symbol,event):
...
def setScrollerTypeValues(touchScrollerType):
    """Populate the scroller type symbol 
    Arguments:
...
def setNumSegmentValues(touchScrollerNum,mutualChannels):
...
def setNumChannelsValues(touchScrollerNumChannels,mutualChannels):
    """Populate the scroller number of segments symbol 
    Arguments:
...
def setResolutionValues(touchScrollerResolution):
...
def setDeadBandValues(touchScrollerDeadband):
...
def setPositionHysteresisValues(touchScrollerPosHysteresis):
...
def setContactThresholdValues(touchScrollerContactThreshold):
