...
"""
import node_groups
def initSurfaceInstance(qtouchComponent, parentLabel , targetDevice, touchKeyCountMax):
    ...
    enableSurfaceMenu.setDependencies(surfaceUpdateEnabled,["ENABLE_SURFACE"])
...
def setEnableSurface1TValues(enableSurface1T): 
    ...
    enableSurface1T.setDependencies(enableSurfaceFiles,["ENABLE_SURFACE1T"])

def setEnableSurface2TValues(enableSurface2T):
    ...
    enableSurface2T.setDependencies(enableSurfaceFiles,["ENABLE_SURFACE2T"])

def setHoriStartKeyValues(horiStartKey, touchKeyCountMax):
...
def setHoriNumKeyValues(horiNumKey):
...
def setVertStartKeyValues(vertStartKey, touchKeyCountMax): 
...
def setVertNumKeyValues(vertNumKey): 
...
def setPositionResolValues(positionResol):
...
#Surface Deadband
def setDeadbandPercentValues(deadbandPercent):
...
#median filter
def setMedianFilterValues(medianFilter): 
...
#iir filter
def setIIRFilterValues(iirFilter):
...
#Position Hysterisis
def setPositionHysteresisValues(posHysterisis):
...
def setContactMinThresholdValues(contactMinThreshold):
...
def enableSurfaceFiles(symbol,event):
...
def getSurfaceRearrangeRequired(targetDevice):
    ...
    Returns:
        :True / False
    """
    surface_rearrangement_macro = set(["SAML10","SAML1xE","SAML11"])

    if(targetDevice in surface_rearrangement_macro):
        return True
    else:
        return False

def updateLumpModeSurface(symbol,touchSenseTechnology,totalChannelCount):
    ...
    if (touchSenseTechnology.getValue() == 1) and (surfaceEnabled == True):
            ...
            for i in range(HORI_START_KEY,(HORI_START_KEY+HORI_NUM_KEY)):
                vals = node_groups.tchMutXPinSelection[int(i)].getValue()
                MUTL_SURFACE_X.append(node_groups.tchMutXPinSelection[int(i)].getKeyValue(vals))
            MUTL_SURFACE_X = "|".join(MUTL_SURFACE_X)
            for j in range(VERT_START_KEY,(VERT_START_KEY+VERT_NUM_KEY)):
                vals = node_groups.tchMutYPinSelection[int(j)].getValue()
                MUTL_SURFACE_Y.append(node_groups.tchMutYPinSelection[int(j)].getKeyValue(vals))
            MUTL_SURFACE_Y = "|".join(MUTL_SURFACE_Y)
            if (VERT_START_KEY >0):
                for i in range(0,VERT_START_KEY):
                    vals1 = node_groups.tchMutXPinSelection[int(i)].getValue()
                    vals2 = node_groups.tchMutYPinSelection[int(i)].getValue()
                    node_groups.tchMutXPinSelection[int(i)].setValue(vals1)
                    node_groups.tchMutYPinSelection[int(i)].setValue(vals2)
            for i in range(VERT_START_KEY,(VERT_START_KEY+VERT_NUM_KEY)):
                vals1 = node_groups.tchMutXPinSelection[int(i)].getValue()
                vals2 = node_groups.tchMutYPinSelection[int(i)].getValue()
                node_groups.tchMutXPinSelection[int(i)].setKeyValue(str(vals1),MUTL_SURFACE_X)
                node_groups.tchMutXPinSelection[int(i)].setValue(vals1)
                node_groups.tchMutYPinSelection[int(i)].setValue(vals2)
            for i in range(HORI_START_KEY,(HORI_START_KEY+HORI_NUM_KEY)):
                vals1 = node_groups.tchMutXPinSelection[int(i)].getValue()
                vals2 = node_groups.tchMutYPinSelection[int(i)].getValue()
                node_groups.tchMutYPinSelection[int(i)].setKeyValue(str(vals2),MUTL_SURFACE_Y)
                node_groups.tchMutXPinSelection[int(i)].setValue(vals1)
                node_groups.tchMutYPinSelection[int(i)].setValue(vals2)
...
def surface_rearrange(symbol,event):
    ...
    if getSurfaceRearrangeRequired(targetDevice) and (touchSenseTechnology == 1) and (surfaceEnabled == True):
        x_append_x_x = []
        x_append_x_y = []
        y_append_y_x = []
        y_append_y_y = []
