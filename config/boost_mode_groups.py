...
def initBoostModeGroup(configName, qtouchComponent, parentMenu, minVal, maxVal, targetDevice):
    """
    ...
    for groupNum in range (minVal,maxVal+1):
        if groupNum ==1:
            ...
            initBoostModeInstance(qtouchComponent,groupNum,boostMenu)
        else:
            ...
            initBoostModeInstance(qtouchComponent,groupNum,vars()[dynamicName])

def initBoostModeInstance(qtouchComponent,groupNumber,parentLabel):
...
def getBoostSupported(targetDevice):
    """
    Checks if the target device supports boost mode
    Arguments : targetDevice
    Returns : True / False
    """
    boost_mode_support = set(["SAML10","SAML1xE","SAML11","PIC32CMLE00","PIC32CMLS00"])
    if(targetDevice in boost_mode_support):
        return True
    else:
        return False

def processBoostMode(symbol,event,targetDevice,nodeCount):
    ...
    if (getBoostSupported(targetDevice)):
