...
def initSurface2DUtilityInstance(configName, qtouchComponent, parentLabel , targetDevice, touchKeyCountMax):
    ...
    enableSurfaceUtilityMenu.setDependencies(enable2DSurfaceFiles,["ENABLE_KRONOCOMM"])

    fileList = []
    fileList.append(set2DUartHeaderFile(configName, qtouchComponent))
    fileList.append(set2DAdapterHeaderFile(configName, qtouchComponent))
    fileList.append(set2DUartSourceFile(configName, qtouchComponent))
    fileList.append(set2DAdapterSourceFile(configName, qtouchComponent))
    return fileList

def set2DUartHeaderFile(configName, qtouchComponent):
    """
...
def set2DAdapterHeaderFile(configName, qtouchComponent):
    """
    Generates 1 touch Surface library file
        :configName : see Variables.get("__CONFIGURATION_NAME")  on MHC api documentation <http://confluence.microchip.com/display/MH/MHC+Python+Interface>
        :qtouchComponent : touchModule
...
def set2DUartSourceFile(configName, qtouchComponent):
    """
    Generates 1 touch Surface library file
        :configName : see Variables.get("__CONFIGURATION_NAME")  on MHC api documentation <http://confluence.microchip.com/display/MH/MHC+Python+Interface>
        :qtouchComponent : touchModule
...
def set2DAdapterSourceFile(configName, qtouchComponent):
    """
    Generates 1 touch Surface library file
        :configName : see Variables.get("__CONFIGURATION_NAME")  on MHC api documentation <http://confluence.microchip.com/display/MH/MHC+Python+Interface>
        :qtouchComponent : touchModule
...
def enable2DSurfaceFiles(symbol,event):
