...
def getMaxGroups():
    """Get maximum Key groups
    Arguments:
        :none
    Returns:
        :number of Key groups  as (int)
    """
    global maxGroups
    return int(maxGroups)

def setMaxGroups(newMax):
    """Set maximum Key groups
    Arguments:
...
#instance
def initKeysGroupInstance(qtouchComponent,groupNumber,parentLabel,selfChannels,mutualChannels):
    ...
    if touchKeyNumChannel != 0:
        ...
        for channelID in range(0, mutualChannels):
            ...
            setThresholdValues(touchKeyDetectThreshold)
            setHysteresisValues(touchKeyHysteresis)
            setAKSValues(touchKeyAKS)

#group
def initKeyGroup(qtouchComponent, touchMenu, minVal,maxVal,selfChannels,mutualChannels):
    ...
    for groupNum in range (minVal,maxVal+1):
        if groupNum ==1:
            ...
            initKeysGroupInstance(qtouchComponent,groupNum,keyMenu,selfChannels,mutualChannels)
        else:
            ...
            initKeysGroupInstance(qtouchComponent,groupNum,vars()[dynamicName],selfChannels,mutualChannels)

#updater
def updateKeyGroups(symbol,event):
...
#parameter assignment
def setThresholdValues(touchKeyDetectThreshold):
...
def setHysteresisValues(touchKeyHysteresis):
