...
def getTchSelfPinSelection():
    ...
    """
    return tchSelfPinSelection

def setTchSelfPinSelection(newTchSelfPinSelection):
    ...
    Returns:
        : none
    """
    tchSelfPinSelection = newTchSelfPinSelection



def getTchMutXPinSelection():
    ...
    """
    return tchMutXPinSelection

def setTchMutXPinSelection(newTchMutXPinSelection):
    ...
    Returns:
        : none
    """
    tchMutXPinSelection = newTchMutXPinSelection

tchMutYPinSelection = []

def getTchMutYPinSelection():
    ...
    """
    return tchMutYPinSelection

def setTchMutYPinSelection(newTchMutYPinSelection):
    ...
    Returns:
        : none
    """
    tchMutYPinSelection = newTchMutYPinSelection
...
def initNodeGroupInstance(qtouchComponent,groupNumber,parentLabel,selfChannels,mutualChannels,ptcPinValues,csdMode,rSelMode):
    ...
    for channelID in range(0, mutualChannels):
        if int(groupNumber) == 1:
            ...
            tchSelfPinSelection.append(qtouchComponent.createKeyValueSetSymbol("GROUP_"+str(groupNumber)+"_SELFCAP-INPUT_"+ str(channelID), vars()[dynamicTouchChEnable]))
            tchMutXPinSelection.append(qtouchComponent.createKeyValueSetSymbol("GROUP_"+str(groupNumber)+"_MUTL-X-INPUT_"+ str(channelID), vars()[dynamicTouchChEnable]))
            tchMutYPinSelection.append(qtouchComponent.createKeyValueSetSymbol("GROUP_"+str(groupNumber)+"_MUTL-Y-INPUT_"+ str(channelID), vars()[dynamicTouchChEnable]))
            #Charge Share Delay
            if(csdMode!="NoCSD"):
        ...
        tchSelfPinSelection[len(tchSelfPinSelection)-1].setLabel("Select Y Pin for Channel "+ str(channelID))
        tchSelfPinSelection[len(tchSelfPinSelection)-1].setOutputMode("Value")
        tchSelfPinSelection[len(tchSelfPinSelection)-1].setDisplayMode("Description")
        tchMutXPinSelection[len(tchSelfPinSelection)-1].setLabel("Select X Pin for Channel "+ str(channelID))
        tchMutXPinSelection[len(tchMutXPinSelection)-1].setOutputMode("Value")
        tchMutXPinSelection[len(tchMutXPinSelection)-1].setDisplayMode("Description")
        tchMutYPinSelection[len(tchSelfPinSelection)-1].setLabel("Select Y Pin for Channel "+ str(channelID))
        tchMutYPinSelection[len(tchMutYPinSelection)-1].setOutputMode("Value")
        tchMutYPinSelection[len(tchMutYPinSelection)-1].setDisplayMode("Description")
        
        if(csdMode!="NoCSD"):
            setCSDValues(touchChargeShareDelay,csdMode)

        if(rSelMode == "e5x"):
            setSeriesRValuesE5x(touchSeriesResistor)
        elif(rSelMode == "l22"):
            setSeriesRValuesL22(touchSeriesResistor)
        else:
            setSeriesRValues(touchSeriesResistor)

        #PTC Clock Prescaler
        setPTCPresscalerValues(touchPTCPrescaler)
        #Analog Gain
        setAnalogGainValues(touchAnalogGain)
        #Digital Filter Gain - Accumulated sum is scaled to Digital Gain
        setDigitalFilterGainValues(touchDigitalFilterGain)
        #Digital Filter Oversampling - Number of samples for each measurement
        setDigitalFilterOversamplingValues(touchDigitalFilterOversampling)
        # X and Y assignment
        for index in range(0, len(ptcPinValues)):
            if(ptcPinValues[index].getAttribute("group") == "Y"):
                tchSelfPinSelection[len(tchSelfPinSelection)-1].addKey(
                ...
                tchMutYPinSelection[len(tchSelfPinSelection)-1].addKey(
                    ptcPinValues[index].getAttribute("index"),ptcPinValues[index].getAttribute("group")+"("+ptcPinValues[index].getAttribute("index")+")",
                    ptcPinValues[index].getAttribute("group")+ptcPinValues[index].getAttribute("index")+ "  ("+ ptcPinValues[index].getAttribute("pad")+")")

            if(ptcPinValues[index].getAttribute("group") == "X"):        
                tchMutXPinSelection[len(tchSelfPinSelection)-1].addKey(
                    ptcPinValues[index].getAttribute("index"),ptcPinValues[index].getAttribute("group")+"("+ptcPinValues[index].getAttribute("index")+")",
                    ptcPinValues[index].getAttribute("group")+ptcPinValues[index].getAttribute("index")+ "  ("+ ptcPinValues[index].getAttribute("pad")+")")


def initNodeGroup(qtouchComponent, touchMenu, minVal,maxVal,selfChannels,mutualChannels, ptcPinValues,csdMode,rSelMode):
    ...
    for groupNum in range (minVal,maxVal+1):
        if groupNum ==1:
            ...
            initNodeGroupInstance(qtouchComponent,groupNum,nodeMenu,selfChannels,mutualChannels,ptcPinValues,csdMode,rSelMode)
        else:
            ...
            initNodeGroupInstance(qtouchComponent,groupNum,vars()[dynamicNodeMenu],selfChannels,mutualChannels,ptcPinValues,csdMode,rSelMode)

def updateNodeGroups(symbol,event):
...
def setCSDValues(touchChargeShareDelay,csdMode):
...
def setSeriesRValues(touchSeriesResistor):
...
def setSeriesRValuesE5x(touchSeriesResistor):
...
def setSeriesRValuesL22(touchSeriesResistor):
...
def setPTCPresscalerValues(touchPTCPrescaler):
...
def setAnalogGainValues(touchAnalogGain):
...
def setDigitalFilterGainValues(touchDigitalFilterGain):
...
def setDigitalFilterOversamplingValues(touchDigitalFilterOversampling):
...
def updateLumpMode(lump_symbol,touchtech):
    ...
    if (lump_feature != ""):
        ...
        print("numLumps = " +str(num_of_lumps))
