...
def lowPowerEventsSupported(targetDevice):
    """
    Checks if the target device supports Low Power events
    Arguments : targetDevice
    Returns : True / False
    """
    LOW_POWER_EVENTS_SUPPORTED_DEVICES = set(
    ["SAMD20","SAMD21","SAMDA1","SAMHA1",
    "SAML11","SAML10",
    "SAMC21","SAMC20",
    "PIC32CMLE00","PIC32CMLS00",
    "SAML21","SAML22"])
    
    if( targetDevice in LOW_POWER_EVENTS_SUPPORTED_DEVICES):
        return True
    else:
        return False

def lowPowerSupported(targetDevice):
    """
    Checks if the target device supports Low Power operation
    Arguments : targetDevice
    Returns : True / False
    """
    LOW_POWER_SUPPORTED_DEVICES = set(
        ["SAMD20","SAMD21","SAMDA1","SAMHA1",
        "SAML11","SAML10",
        "SAMC21","SAMC20",
        "PIC32CMLE00","PIC32CMLS00",
    ...
    if( targetDevice in LOW_POWER_SUPPORTED_DEVICES):
        return True
    else:
        return False

def initLowPowerInstance(qtouchComponent, parentLabel , targetDevice):
    """
    ...
    if (lowPowerEventsSupported(targetDevice)):
    ...
    setLowPowerKeyValues(qtouchComponent,LowPowerEvntMenu)
    setLowPowerDetThreshold(qtouchComponent,LowPowerEvntMenu)
    # setLowPowerPeriod(qtouchComponent,LowPowerEvntMenu,targetDevice)
    setLowPowerTriggerPeriod(qtouchComponent,LowPowerEvntMenu)
    setTouchInactivityTime(qtouchComponent,LowPowerEvntMenu)
    setDriftPeriod(qtouchComponent,LowPowerEvntMenu)
    setLowPowerKeymask(qtouchComponent,LowPowerEvntMenu)


def setLowPowerKeyValues(qtouchComponent,LowPowerEvntMenu):
...
def setLowPowerDetThreshold(qtouchComponent,LowPowerEvntMenu):
...
def setLowPowerPeriod(qtouchComponent,LowPowerEvntMenu,targetDevice):
...
def setLowPowerTriggerPeriod(qtouchComponent,LowPowerEvntMenu):
...
def setTouchInactivityTime(qtouchComponent,LowPowerEvntMenu):
...
def setDriftPeriod(qtouchComponent,LowPowerEvntMenu):
...
def setLowPowerKeymask(qtouchComponent,LowPowerEvntMenu):
